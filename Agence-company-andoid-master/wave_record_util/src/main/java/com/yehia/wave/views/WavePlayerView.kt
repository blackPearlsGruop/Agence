...
import com.yehia.wave.views.Permission.showPermission2
...
//        setColor(btnIcon, R.color.colorPrimary)
        ...
        mPlayButton?.setOnClickListener {
            activity?.showPermission2({
                                    ...
                                    if (!mPlayerAdapter!!.hasTarget(mTarget)) {
                                        val urlFile = when (mTarget!!.targetType) {
                                            ...
                                            PlayerTarget.Type.LOCAL_FILE_URI -> {
                                                Log.e(
                                                    "MEDIAPLAY_HOLDER_TAG",
                                                    "Type is LOCAL_FILE_URI"
                                                )
                ...
                Toast.makeText(
                    activity,
                    activity?.getString(R.string.the_sound_will_not_be_changed_without_permission),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
