...
import com.ksa.agenceCompany.databinding.FragmentBottomSheetSendValueBinding
...
class BottomSheetSendValueFragment : BaseBottomDialog<FragmentBottomSheetSendValueBinding>() {

    override fun getLayoutId(): Int = R.layout.fragment_bottom_sheet_send_value
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (arguments != null) {
            ...
            val args: BottomSheetSendValueFragmentArgs =
                BottomSheetSendValueFragmentArgs.fromBundle(requireArguments())
            idOpportunity = args.idOpp
    ...
    private fun initResponse() {
        ...
        homeViewModel.sendRequestResponse.observe(viewLifecycleOwner, Observer { result ->
            when (result) {
                is Resource.Success -> {
                    showProgress(false)
                    result.data?.let { it ->
    ...
    private fun onClick() {
        mViewDataBinding.btnSend.setOnClickListener {
            ...
            if (price.isEmpty()) {
                ...
                homeViewModel.sendRequest(idOpportunity, price.toInt())
