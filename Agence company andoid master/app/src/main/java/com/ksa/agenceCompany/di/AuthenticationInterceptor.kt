...
import com.ksa.agenceCompany.common.BEARER
...
class AuthenticationInterceptor(private val context: Context) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        val token = BEARER + " " + AgenceCompanyApp.pref.authToken.toString()
        if ( Locale.getDefault().language =="ar")
        {
            lang = AgenceCompanyApp.pref.getString(LANG, "ar").toString()

        }
        else{
            lang = AgenceCompanyApp.pref.getString(LANG, "en").toString()
